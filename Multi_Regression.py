...
class RegressioninputCheck:
  ...
  def array_batch_check(array_list):
    if type(array_list) == np.ndarray:
      multiple_num_list = np.array([1] * 1 if len(array_list.shape) <= 2 else array_list.shape[0], dtype=int)
      return array_list, multiple_num_list
    else:
      # Judgement of sub list in the whole list
      has_list = any(isinstance(sub_item, list) and len(np.array(sub_item).shape) > 1 for sub_item in array_list)
      
      if has_list:
        sub_list_length = [len(sub_list_item) for sub_list_item in array_list]
        least_common_multiple = np.lcm.reduce(sub_list_length)  # Getting the least common multiple
        multiple_num_list = [least_common_multiple] * len(sub_list_length) / np.array(sub_list_length, dtype=int)
        array_list = [array_list[index] * int(multiple_num_list[index]) for index in range(len(sub_list_length))]
      else:
  ...
  def __init__(self, strain_variable, independent_variable, batch_check):
    ...
    if type(self.strain_variable) == np.ndarray and type(self.independent_variable) == np.ndarray:
      self.batch_check = False
      # if推导表达式
      self.multiple_num_list = np.array([1] * 1 if len(independent_variable.shape) <= 2 else independent_variable.shape[0], dtype=int)
    ...
    if self.batch_check:
      ...
      self.strain_variable, _ = self.array_bacth_check(self.strain_variable)
      self.independent_variable, self.multiple_num_list = self.array_batch_check(self.independent_variable)
  ...
  def check_logic(self):
    ...
    if self.dim_s_v_length > 1 and self.dim_s_v[-1] != 1:
      print("The last axis of strain varaible must be 1, you got {}".format(self.dim_s_v[-1]))
    ...
    try:
      if self.dim_s_v_length < max_dim:
        self.strain_variable = self.strain_variable.reshape(self.dim_s_v[: -1].__add__((1,)))
    ...
    while self.dim_s_v_length < 3 or self.dim_i_v_length < 3:
      ...
      self.independent_variable = np.expand_dims(self.independent_variable, axis=0)
      self.dim_s_v_length = len(self.strain_variable.shape)
      self.dim_i_v_length = len(self.strain_variable.shape)
...
def regression(strain_variable, independent_variable, with_constant=False, batch_check=False, confidence_level=0.95):
  ...
  # scaled.（等比例缩放）
  var = np.einsum('...ii->...i',
                  RCM * np.linalg.inv(np.matmul(i_v_t, i_v) / np.expand_dims(multiple_num_list, axis=-1))
  ...
  SST = np.sum(np.square(s_v - np.expand_dims(np.mean(s_v, axis=1), axis=-1)), axis=1) / multiple_num_list
  ...
  p_value_T = st.t.sf(np.abs(t_stat), np.expand_dims(df_residual, axis=-1)) * 2
